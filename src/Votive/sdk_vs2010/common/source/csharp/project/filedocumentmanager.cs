//-------------------------------------------------------------------------------------------------
// <copyright file="filedocumentmanager.cs" company="Outercurve Foundation">
...
namespace Microsoft.VisualStudio.Package
{
	...
	public class FileDocumentManager : DocumentManager
	{
		...
		private int Open(bool newFile, bool openWith, uint editorFlags, ref Guid editorType, string physicalView, ref Guid logicalView, IntPtr docDataExisting, out IVsWindowFrame windowFrame, WindowFrameShowAction windowFrameAction)
		{
			...
			FileNode nodeAsFileNode = (FileNode)this.Node;
			if (!nodeAsFileNode.IsFileOnDisk(true))
			{
				// Inform clients that we have an invalid item (wrong icon)
				this.Node.OnInvalidateItems(this.Node.Parent);
				nodeAsFileNode.FilePreviouslyUnavailable = true;
			...
			else if (nodeAsFileNode.FilePreviouslyUnavailable)
			{
				// The file is now available, update the icon
				nodeAsFileNode.FilePreviouslyUnavailable = false;
				this.Node.ReDraw(UIHierarchyElement.SccState | UIHierarchyElement.Icon);
			}
