//-------------------------------------------------------------------------------------------------
// <copyright file="projectnode.cs" company="Outercurve Foundation">
...
namespace Microsoft.VisualStudio.Package
{
        ...
        IVsDeferredSaveProject
    {
        ...
        public virtual void PrepareBuild(string configName, string platformName, bool cleanBuild)
        {
            if (this.buildIsPrepared && !cleanBuild) return;

            ProjectOptions options = this.GetProjectOptions(configName, platformName);
        ...
        public virtual MSBuildResult Build(uint vsopts, string configName, string platformName, IVsOutputWindowPane output, string target)
        {
            lock (ProjectNode.BuildLock)
            {
                ...
                try
                {
                    this.SetBuildConfigurationProperties(configName, platformName);
        ...
        public virtual void InvalidatePropertyCache()
        {
            ...
            this.ConfigProvider.InvalidatePropertyCache();
        }

        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Globalization", "CA1308:NormalizeStringsToUppercase")]
        public virtual ProjectOptions GetProjectOptions(string configName, string platformName)
        {
            ...
            this.SetConfiguration(configName, platformName);
            ...
            options.OutputAssembly = outputPath + this.GetAssemblyName(configName, platformName);
        ...
        public virtual bool GetBoolAttr(string configName, string platformName, string name)
        {
            this.SetConfiguration(configName, platformName);
        ...
        public virtual string GetAssemblyName(string configName, string platformName)
        {
            this.SetConfiguration(configName, platformName);
            MSBuild.BuildPropertyGroup properties = this.buildProject.EvaluatedProperties;
        ...
        protected virtual void SetBuildConfigurationProperties(string configName, string platformName)
        {
            ProjectOptions options = null;

            if (!String.IsNullOrEmpty(configName) && !String.IsNullOrEmpty(platformName))
            {
                options = this.GetProjectOptions(configName, platformName);
            }
        ...
        protected virtual HierarchyNode AddNewFileNodeToHierarchy(HierarchyNode parentNode, string fileName, string linkPath, VSADDFILEFLAGS addFlags)
        {
            HierarchyNode child;
            ...
            if ((addFlags & VSADDFILEFLAGS.VSADDFILEFLAGS_AddToSourceControlDoneExternally) == VSADDFILEFLAGS.VSADDFILEFLAGS_NoFlags)
            {
                // TODO : Revisit the VSADDFILEFLAGS here. Can it be a nested project?
                this.tracker.OnItemAdded(fileName, addFlags);
            }
        ...
        protected internal virtual void SetConfiguration(string configName, string platformName)
        {
            ...
            if (this.BuildInProgress)
            {
                ...
                bool configsAreEqual =
                    String.Equals(currentConfigName, configName, StringComparison.OrdinalIgnoreCase) &&
                    String.Equals(currentPlatformName, platformName, StringComparison.OrdinalIgnoreCase);
            ...
            this.buildProject.GlobalProperties.SetProperty(ProjectFileConstants.Configuration, configName);
            this.buildProject.GlobalProperties.SetProperty(ProjectFileConstants.Platform, platformName);
        ...
        protected internal virtual void ProcessFiles()
        {
            ...
            foreach (MSBuild.BuildItem item in projectFiles)
            {
                ...
                if (!String.IsNullOrEmpty(item.GetEvaluatedMetadata(ProjectFileAttributeValue.Visible)))
                {
                    bool result;
                    if (Boolean.TryParse(item.GetEvaluatedMetadata(ProjectFileAttributeValue.Visible), out result) && !result)
                    {
                        continue;
                    }
                }
        ...
        /// Resumes MSBuild.
        /// </summary>
        public void ResumeMSBuild(string configName, string platformName, IVsOutputWindowPane output, string target)
        {
            this.suspendMSBuildCounter--;
            ...
            if (this.suspendMSBuildCounter == 0 && this.invokeMSBuildWhenResumed)
            {
                try
                {
                    this.Build(configName, platformName, output, target);
                }
                finally
                {
        ...
        /// Resumes MSBuild.
        /// </summary>
        public void ResumeMSBuild(string configName, string platformName, string target)
        {
            this.ResumeMSBuild(configName, platformName, null, target);
        }
        ...
        /// </summary>
        public MSBuildResult CallMSBuild(string configName, string platformName, IVsOutputWindowPane output, string target)
        {
            ...
            {
                return this.Build(configName, platformName, output, target);
            }
        }
        ...
        /// </summary>
        public MSBuildResult CallMSBuild(string configName, string platformName, string target)
        {
            return this.CallMSBuild(configName, platformName, null, target);
        }
        ...
        /// </summary>
        public MSBuildResult Build(string configName, string platformName, IVsOutputWindowPane output, string target)
        {
            return this.Build(0, configName, platformName, output, target);
        }

        /// <summary>
        /// Overloaded method to invoke MSBuild. Does not log build results to the output window pane.
        /// </summary>
        public MSBuildResult Build(string configName, string platformName, string target)
        {
            return this.Build(0, configName, platformName, null, target);
        }
        ...
        public string GetOutputPath(string configName, string platformName)
        {
            this.SetConfiguration(configName, platformName);
            MSBuild.BuildPropertyGroup properties = this.buildProject.EvaluatedProperties;
        ...
        public string GetOutputAssembly(string configName, string platformName)
        {
            ProjectOptions options = this.GetProjectOptions(configName, platformName);
        ...
        {
            return this.AddItemWithSpecific(itemIdLoc, op, VSADDFILEFLAGS.VSADDFILEFLAGS_NoFlags, itemName, filesToOpen, files, dlgOwner, editorFlags, ref editorType, physicalView, ref logicalView, result);
        }

        internal int AddItemWithSpecific(uint itemIdLoc, VSADDITEMOPERATION op, VSADDFILEFLAGS addFlags, string itemName, uint filesToOpen, string[] files, IntPtr dlgOwner, uint editorFlags, ref Guid editorType, string physicalView, ref Guid logicalView, VSADDRESULT[] result)
        {
            ...
            for (int index = 0; index < filesToAdd.Count; index++)
            {
                ...
                if (op == VSADDITEMOPERATION.VSADDITEMOP_LINKTOFILE)
                {
                    ...
                    this.AddNewFileNodeToHierarchy(n, relativePath, linkPath, addFlags);
                }
                else
                {
                    ...
                    if (existingLink != null)
                    {
                        ...
                        newFileNode = this.AddNewFileNodeToHierarchy(n, newFileName, null, addFlags);
                    }
                    else if (overwrite)
                    {
        ...
        private HierarchyNode AddIndependentFileNode(MSBuild.BuildItem item)
        {
            ...
            string includePath = item.Include.Replace("/./", "/").Replace("\\.\\", "\\");
            ...
            if (includePath != item.Include)
            {
                item.Include = includePath;
            }

            // Make sure the item is within the project folder hierarchy. If not, link it.
            string linkPath = item.GetMetadata(ProjectFileConstants.Link);
            if (String.IsNullOrEmpty(linkPath))
            {
                string projectFolder = new Uri(this.ProjectFolder).LocalPath;
                string itemPath = new Uri(Path.Combine(this.ProjectFolder, item.Include)).LocalPath;
                if (!itemPath.StartsWith(projectFolder, StringComparison.OrdinalIgnoreCase))
                {
                    linkPath = Path.GetFileName(item.Include);
                    item.SetMetadata(ProjectFileConstants.Link, linkPath);
                }
            }
        ...
        public MSBuild.BuildProperty GetMsBuildProperty(string propertyName, bool resetCache, string configName, string platformName)
        {
            if (resetCache || this.currentConfig == null)
            {
                // Get properties from project file and cache it
            ...
            // Make sure that the cache is not 'corrupted' by this call for an explicit configuration
            MSBuild.BuildProperty property = this.currentConfig[propertyName];
            this.currentConfig = null;
            return property;
        }
